...
export function DataState(state: Array<SingleFont> = State.dataState, action: AppActions.DataActions | AppActions.UIActions): Array<SingleFont> {
  switch (action.type) {
    ...
    case AppActions.FILTER_NUMBER_OF_STYLES:
      const numberOfStyles = action.payload;

      return state.map((font: SingleFont) => {
        const variants = font.variants.length;
        if (numberOfStyles <= variants) {
          font.currentState.visible = true;
        } else {
          font.currentState.visible = false;
        }
        return font;
      });
    case AppActions.CHANGE_VISIBLE_FONTS:
